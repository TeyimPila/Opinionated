...
const ShowValues = props => {
  ...
  const Succeeded = () => {
    ...
    } else return <div>No Success Yet - keep trying</div>;
  };
  ...
  const Failed = () => {
    ...
    } else return <div>No fail yet</div>;
  };
  ...
  const Warnings = () => {
    ...
    } else return <div>No warnings yet</div>;
  };
  ...
  return (
    <div className="well well-sm" style={style}>
      ...
      <p>
        It exists simple to show how the Values, Warnings and Errors can be
        accessed
      </p>
      <p>
        Most of the fields have some validations or warning triggers, see the
        code in MyForm
      </p>
      <Values />
      <Succeeded />
...
export default connect(mapStateToProps)(ShowValues);
